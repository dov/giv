// Literal copy of gregex in order to support glib 2.10.

/* GivRegex -- regular expression API wrapper around PCRE.
 *
...
/* Mask of all the possible values for GivRegexCompileFlags. */
#define GIV_REGEX_COMPILE_MASK (GIV_REGEX_CASELESS		| \
			      GIV_REGEX_MULTILINE		| \
			      GIV_REGEX_DOTALL		| \
			      GIV_REGEX_EXTENDED		| \
			      GIV_REGEX_ANCHORED		| \
			      GIV_REGEX_DOLLAR_ENDONLY	| \
			      GIV_REGEX_UNGREEDY		| \
			      GIV_REGEX_RAW		| \
			      GIV_REGEX_NO_AUTO_CAPTURE	| \
			      GIV_REGEX_OPTIMIZE		| \
			      GIV_REGEX_DUPNAMES		| \
			      GIV_REGEX_NEWLINE_CR	| \
			      GIV_REGEX_NEWLINE_LF	| \
			      GIV_REGEX_NEWLINE_CRLF)

/* Mask of all the possible values for GivRegexMatchFlags. */
#define GIV_REGEX_MATCH_MASK (GIV_REGEX_MATCH_ANCHORED	| \
			    GIV_REGEX_MATCH_NOTBOL	| \
			    GIV_REGEX_MATCH_NOTEOL	| \
			    GIV_REGEX_MATCH_NOTEMPTY	| \
			    GIV_REGEX_MATCH_PARTIAL	| \
			    GIV_REGEX_MATCH_NEWLINE_CR	| \
			    GIV_REGEX_MATCH_NEWLINE_LF	| \
			    GIV_REGEX_MATCH_NEWLINE_CRLF	| \
			    GIV_REGEX_MATCH_NEWLINE_ANY)
...
struct _GivMatchInfo
{
  GivRegex *regex;		/* the regex */
  GivRegexMatchFlags match_opts;	/* options used at match time on the regex */
...
struct _GivRegex
{
  ...
  GivRegexCompileFlags compile_opts;	/* options used at compile time on the pattern */
  GivRegexMatchFlags match_opts;	/* options used at match time on the regex */
  pcre_extra *extra;		/* data stored when GIV_REGEX_OPTIMIZE is used */
};
...
static gboolean	 interpolate_replacement	(const GivMatchInfo *match_info,
						 GString *result,
						 gpointer data);
...
match_error (gint errcode)
{
  switch (errcode)
    ...
    case PCRE_ERROR_NULL:
      /* NULL argument, this should not happen in GivRegex */
    ...
    case PCRE_ERROR_BADCOUNT:
      /* negative ovecsize, this should not happen in GivRegex */
    ...
    case PCRE_ERROR_DFA_UMLIMIT:
      /* the match_field field is not used in GivRegex */
      break;
...
translate_compile_error (gint *errcode, const gchar **errmsg)
{
   ...
   * GIV_REGEX_ERROR_COMPILE error code in errcode and keep the
   * untranslated error message returned by PCRE.
   * Note that there can be more PCRE errors with the same GivRegexError
  ...
  *errcode += 100;
  ...
  switch (*errcode)
    {
    case GIV_REGEX_ERROR_STRAY_BACKSLASH:
      *errmsg = _("\\ at end of pattern");
      break;
    case GIV_REGEX_ERROR_MISSING_CONTROL_CHAR:
      *errmsg = _("\\c at end of pattern");
      break;
    case GIV_REGEX_ERROR_UNRECOGNIZED_ESCAPE:
    ...
    case 137:
       ...
       */
      *errcode = GIV_REGEX_ERROR_UNRECOGNIZED_ESCAPE;
      *errmsg = _("case-changing escapes (\\l, \\L, \\u, \\U) are not allowed here");
      break;
    case GIV_REGEX_ERROR_QUANTIFIERS_OUT_OF_ORDER:
      *errmsg = _("numbers out of order in {} quantifier");
      break;
    case GIV_REGEX_ERROR_QUANTIFIER_TOO_BIG:
      *errmsg = _("number too big in {} quantifier");
      break;
    case GIV_REGEX_ERROR_UNTERMINATED_CHARACTER_CLASS:
      *errmsg = _("missing terminating ] for character class");
      break;
    case GIV_REGEX_ERROR_INVALID_ESCAPE_IN_CHARACTER_CLASS:
      *errmsg = _("invalid escape sequence in character class");
      break;
    case GIV_REGEX_ERROR_RANGE_OUT_OF_ORDER:
      *errmsg = _("range out of order in character class");
      break;
    case GIV_REGEX_ERROR_NOTHING_TO_REPEAT:
      *errmsg = _("nothing to repeat");
      break;
    case GIV_REGEX_ERROR_UNRECOGNIZED_CHARACTER:
      *errmsg = _("unrecognized character after (?");
      break;
    case 124:
      *errcode = GIV_REGEX_ERROR_UNRECOGNIZED_CHARACTER;
      *errmsg = _("unrecognized character after (?<");
      break;
    case 141:
      *errcode = GIV_REGEX_ERROR_UNRECOGNIZED_CHARACTER;
      *errmsg = _("unrecognized character after (?P");
      break;
    case GIV_REGEX_ERROR_POSIX_NAMED_CLASS_OUTSIDE_CLASS:
      *errmsg = _("POSIX named classes are supported only within a class");
      break;
    case GIV_REGEX_ERROR_UNMATCHED_PARENTHESIS:
      *errmsg = _("missing terminating )");
      break;
    case 122:
      *errcode = GIV_REGEX_ERROR_UNMATCHED_PARENTHESIS;
    ...
    case 129:
      ...
      *errmsg = _("(?R or (?[+-]digits must be followed by )");
      break;
    case GIV_REGEX_ERROR_INEXISTENT_SUBPATTERN_REFERENCE:
      *errmsg = _("reference to non-existent subpattern");
      break;
    case GIV_REGEX_ERROR_UNTERMINATED_COMMENT:
      *errmsg = _("missing ) after comment");
      break;
    case GIV_REGEX_ERROR_EXPRESSION_TOO_LARGE:
      *errmsg = _("regular expression too large");
      break;
    case GIV_REGEX_ERROR_MEMORY_ERROR:
      *errmsg = _("failed to get memory");
      break;
    case GIV_REGEX_ERROR_VARIABLE_LENGTH_LOOKBEHIND:
      *errmsg = _("lookbehind assertion is not fixed length");
      break;
    case GIV_REGEX_ERROR_MALFORMED_CONDITION:
      *errmsg = _("malformed number or name after (?(");
      break;
    case GIV_REGEX_ERROR_TOO_MANY_CONDITIONAL_BRANCHES:
      *errmsg = _("conditional group contains more than two branches");
      break;
    case GIV_REGEX_ERROR_ASSERTION_EXPECTED:
      *errmsg = _("assertion expected after (?(");
      break;
    case GIV_REGEX_ERROR_UNKNOWN_POSIX_CLASS_NAME:
      *errmsg = _("unknown POSIX class name");
      break;
    case GIV_REGEX_ERROR_POSIX_COLLATING_ELEMENTS_NOT_SUPPORTED:
      *errmsg = _("POSIX collating elements are not supported");
      break;
    case GIV_REGEX_ERROR_HEX_CODE_TOO_LARGE:
      *errmsg = _("character value in \\x{...} sequence is too large");
      break;
    case GIV_REGEX_ERROR_INVALID_CONDITION:
      *errmsg = _("invalid condition (?(0)");
      break;
    case GIV_REGEX_ERROR_SINGLE_BYTE_MATCH_IN_LOOKBEHIND:
      *errmsg = _("\\C not allowed in lookbehind assertion");
      break;
    case GIV_REGEX_ERROR_INFINITE_LOOP:
      *errmsg = _("recursive call could loop indefinitely");
      break;
    case GIV_REGEX_ERROR_MISSING_SUBPATTERN_NAME_TERMINATOR:
      *errmsg = _("missing terminator in subpattern name");
      break;
    case GIV_REGEX_ERROR_DUPLICATE_SUBPATTERN_NAME:
      *errmsg = _("two named subpatterns have the same name");
      break;
    case GIV_REGEX_ERROR_MALFORMED_PROPERTY:
      *errmsg = _("malformed \\P or \\p sequence");
      break;
    case GIV_REGEX_ERROR_UNKNOWN_PROPERTY:
      *errmsg = _("unknown property name after \\P or \\p");
      break;
    case GIV_REGEX_ERROR_SUBPATTERN_NAME_TOO_LONG:
      *errmsg = _("subpattern name is too long (maximum 32 characters)");
      break;
    case GIV_REGEX_ERROR_TOO_MANY_SUBPATTERNS:
      *errmsg = _("too many named subpatterns (maximum 10,000)");
      break;
    case GIV_REGEX_ERROR_INVALID_OCTAL_VALUE:
      *errmsg = _("octal value is greater than \\377");
      break;
    case GIV_REGEX_ERROR_TOO_MANY_BRANCHES_IN_DEFINE:
      *errmsg = _("DEFINE group contains more than one branch");
      break;
    case GIV_REGEX_ERROR_DEFINE_REPETION:
      *errmsg = _("repeating a DEFINE group is not allowed");
      break;
    case GIV_REGEX_ERROR_INCONSISTENT_NEWLINE_OPTIONS:
      *errmsg = _("inconsistent NEWLINE options");
      break;
    case GIV_REGEX_ERROR_MISSING_BACK_REFERENCE:
      *errmsg = _("\\g is not followed by a braced name or an optionally "
		 "braced non-zero number");
      break;
    case 11:
      *errcode = GIV_REGEX_ERROR_INTERNAL;
    ...
    case 16:
      ...
      *errcode = GIV_REGEX_ERROR_COMPILE;
      break;
    case 17:
...
static GivMatchInfo *
match_info_new (const GivRegex *regex,
		const gchar  *string,
		gint          string_len,
		gint          start_position,
		gint          match_options,
		gboolean      is_dfa)
{
  ...
  match_info = g_new0 (GivMatchInfo, 1);
  match_info->regex = giv_regex_ref ((GivRegex *)regex);
  ...
  if (is_dfa)
    {
       ...
       * enough giv_regex_match_all_full() will expand them. */
 ...
 * @match_info: a #GivMatchInfo
 *
 * Returns #GivRegex object used in @match_info. It belongs to Glib
 * and must not be freed. Use giv_regex_ref() if you need to keep it
 * after you free @match_info object.
 *
 * Returns: #GivRegex object used in @match_info
 *
 * Since: 2.14
 */
GivRegex *
giv_match_info_get_regex (const GivMatchInfo *match_info)
{
...
/**
 * giv_match_info_get_string:
 * @match_info: a #GivMatchInfo
 *
 * Returns the string searched with @match_info. This is the
 * string passed to giv_regex_match() or giv_regex_replace() so
 ...
 */
const gchar *
giv_match_info_get_string (const GivMatchInfo *match_info)
{
  g_return_val_if_fail (match_info != NULL, NULL);
 ...
 * @match_info: a #GivMatchInfo
 *
 * Frees all the memory associated with the #GivMatchInfo structure.
 *
 * Since: 2.14
 */
void
giv_match_info_free (GivMatchInfo *match_info)
{
  if (match_info)
    {
      giv_regex_unref (match_info->regex);
...
/**
 * giv_match_info_next:
 * @match_info: a #GivMatchInfo structure
 * @error: location to store the error occuring, or %NULL to ignore errors
 *
 * Scans for the next match using the same parameters of the previous
 * call to giv_regex_match_full() or giv_regex_match() that returned
 * @match_info.
 *
 ...
 */
gboolean
giv_match_info_next (GivMatchInfo  *match_info,
		   GError     **error)
{
  ...
  if (IS_PCRE_ERROR (match_info->matches))
    {
      g_set_error (error, GIV_REGEX_ERROR, GIV_REGEX_ERROR_MATCH,
  ...
  if (match_info->matches >= 0 &&
      ...
      prev_match_end == match_info->offsets[1])
    {
      ...
      return giv_match_info_next (match_info, error);
    }
 ...
 */
gboolean
giv_match_info_matches (const GivMatchInfo *match_info)
{
  g_return_val_if_fail (match_info != NULL, FALSE);
...
/**
 * giv_match_info_get_match_count:
 * @match_info: a #GivMatchInfo structure
 *
 ...
 * using giv_regex_match_all() or giv_regex_match_all_full(), the retrieved
 ...
 */
gint
giv_match_info_get_match_count (const GivMatchInfo *match_info)
{
...
/**
 * giv_match_info_is_partial_match:
 * @match_info: a #GivMatchInfo structure
 *
 * Usually if the string passed to giv_regex_match*() matches as far as
 ...
 *
 * GivRegex supports the concept of partial matching by means of the
 * #GIV_REGEX_MATCH_PARTIAL flag. When this is set the return code for
 * giv_regex_match() or giv_regex_match_full() is, as usual, %TRUE
 * for a complete match, %FALSE otherwise. But, when these functions
 * return %FALSE, you can check if the match was partial calling
 * giv_match_info_is_partial_match().
 *
 * When using partial matching you cannot use giv_match_info_fetch*().
 *
 ...
 * coded thus "(a){2,4}" and "(\d)+". If #GIV_REGEX_MATCH_PARTIAL is set 
 ...
 */
gboolean
giv_match_info_is_partial_match (const GivMatchInfo *match_info)
{
  g_return_val_if_fail (match_info != NULL, FALSE);
...
/**
 * giv_match_info_expand_references:
 * @match_info: a #GivMatchInfo or %NULL
 ...
 * giv_regex_replace().
 *
 * The @string_to_expand must be UTF-8 encoded even if #GIV_REGEX_RAW was
 * passed to giv_regex_new().
 *
 ...
 * Use giv_regex_check_replacement() to find out whether @string_to_expand
 * contains references.
 *
 ...
 */
gchar *
giv_match_info_expand_references (const GivMatchInfo  *match_info, 
  ...
  if (!match_info && interpolation_list_needs_match (list))
    {
      g_critical ("String '%s' contains references to the match, can't "
		  "expand references without GivMatchInfo object",
		  string_to_expand);
      return NULL;
    }
...
/**
 * giv_match_info_fetch:
 * @match_info: #GivMatchInfo structure
 ...
 * giv_regex_match_all() or giv_regex_match_all_full(), the retrieved
 ...
 */
gchar *
giv_match_info_fetch (const GivMatchInfo *match_info,
		    gint              match_num)
{
  ...
  if (!giv_match_info_fetch_pos (match_info, match_num, &start, &end))
    match = NULL;
  else if (start == -1)
    match = g_strdup ("");
  else
 ...
 */
gboolean
giv_match_info_fetch_pos (const GivMatchInfo *match_info,
			gint              match_num,
			gint             *start_pos,
			gint             *end_pos)
{
 ...
 */
static gint
get_matched_substring_number (const GivMatchInfo *match_info,
			      const gchar      *name)
{
  gint entrysize;
  gchar *first, *last;
  ...
  if (!(match_info->regex->compile_opts & GIV_REGEX_DUPNAMES))
 ...
 */
gchar *
giv_match_info_fetch_named (const GivMatchInfo *match_info,
			  const gchar      *name)
{
  ...
  if (num < 0)
    ...
    return giv_match_info_fetch (match_info, num);
}

/**
 * giv_match_info_fetch_named_pos:
 * @match_info: #GivMatchInfo structure
 * @name: name of the subexpression
 * @start_pos: pointer to location where to store the start position
 * @end_pos: pointer to location where to store the end position
 ...
 */
gboolean
giv_match_info_fetch_named_pos (const GivMatchInfo *match_info,
			      const gchar      *name,
			      gint             *start_pos,
			      gint             *end_pos)
{
  ...
  return giv_match_info_fetch_pos (match_info, num, start_pos, end_pos);
}

/**
 * giv_match_info_fetch_all:
 * @match_info: a #GivMatchInfo structure
 *
 ...
 */
gchar **
giv_match_info_fetch_all (const GivMatchInfo *match_info)
{
  ...
  for (i = 0; i < match_info->matches; i++)
    result[i] = giv_match_info_fetch (match_info, i);
  result[i] = NULL;
 ...
 */
void
giv_regex_unref (GivRegex *regex)
{
 ...
 * the initial setup of the #GivRegex structure.  
 * 
 * Returns: a #GivRegex structure. Call giv_regex_unref() when you 
 *   are done with it
 *
 * Since: 2.14
 */
GivRegex *
giv_regex_new (const gchar         *pattern, 
	     ...
	     GError             **error)
{
  ...
  g_return_val_if_fail (error == NULL || *error == NULL, NULL);
  g_return_val_if_fail ((compile_options & ~GIV_REGEX_COMPILE_MASK) == 0, NULL);
  g_return_val_if_fail ((match_options & ~GIV_REGEX_MATCH_MASK) == 0, NULL);
  ...
  if (!initialized)
      ...
      if (!support)
	{
	  ...
	  g_critical ("%s", msg);
	  g_set_error(error, GIV_REGEX_ERROR, GIV_REGEX_ERROR_COMPILE, gettext (msg));
	  return NULL;
	}
  ...
  /* GIV_REGEX_OPTIMIZE has the same numeric value of PCRE_NO_UTF8_CHECK,
   * as we do not need to wrap PCRE_NO_UTF8_CHECK. */
  if (compile_options & GIV_REGEX_OPTIMIZE)
    optimize = TRUE;

  /* In GivRegex the string are, by default, UTF-8 encoded. PCRE
   * instead uses UTF-8 only if required with PCRE_UTF8. */
  if (compile_options & GIV_REGEX_RAW)
    {
      /* disable utf-8 */
      compile_options &= ~GIV_REGEX_RAW;
    }
  else
    {
      /* enable utf-8 */
  ...
  if (!(compile_options & GIV_REGEX_NEWLINE_CR) &&
      !(compile_options & GIV_REGEX_NEWLINE_LF))
    {
  ...
  if (re == NULL)
    {
      ...
      /* Translate the PCRE error code to GivRegexError and use a translated
      ...
      tmp_error = g_error_new (GIV_REGEX_ERROR, errcode,
  ...
  if (!(compile_options & GIV_REGEX_DUPNAMES))
    {
      gboolean jchanged = FALSE;
      pcre_fullinfo (re, NULL, PCRE_INFO_JCHANGED, &jchanged);
      if (jchanged)
	compile_options |= GIV_REGEX_DUPNAMES;
    }

  regex = g_new0 (GivRegex, 1);
  regex->ref_count = 1;
  ...
  if (optimize)
    {
      ...
      if (errmsg != NULL)
        {
          GError *tmp_error = g_error_new (GIV_REGEX_ERROR,
...
/**
 * giv_regex_get_pattern:
 * @regex: a #GivRegex structure
 *
 * Gets the pattern string associated with @regex, i.e. a copy of 
 * the string passed to giv_regex_new().
 *
 ...
 */
const gchar *
giv_regex_get_pattern (const GivRegex *regex)
{
  g_return_val_if_fail (regex != NULL, NULL);
 ...
 */
gint
giv_regex_get_max_backref (const GivRegex *regex)
{
  gint value;
 ...
 */
gint
giv_regex_get_capture_count (const GivRegex *regex)
{
  gint value;
 ...
 *
 * This function is equivalent to giv_regex_match() but it does not
 * require to compile the pattern with giv_regex_new(), avoiding some
 * lines of code when you need just to do a match without extracting
 * substrings, capture counts, and so on.
 *
 * If this function is to be called on the same @pattern more than
 * once, it's more efficient to compile the pattern once with
 * giv_regex_new() and then use giv_regex_match().
 *
 ...
 */
gboolean
giv_regex_match_simple (const gchar        *pattern, 
		      const gchar        *string, 
		      GivRegexCompileFlags  compile_options,
		      GivRegexMatchFlags    match_options)
{
  ...
  regex = giv_regex_new (pattern, compile_options, 0, NULL);
  if (!regex)
    return FALSE;
  result = giv_regex_match_full (regex, string, -1, 0, match_options, NULL, NULL);
  giv_regex_unref (regex);
  return result;
}
...
/**
 * giv_regex_match:
 * @regex: a #GivRegex structure from giv_regex_new()
 * @string: the string to scan for matches
 * @match_options: match options
 * @match_info: pointer to location where to store the #GivMatchInfo, 
 *   or %NULL if you do not need it
 *
 * Scans for a match in string for the pattern in @regex. 
 * The @match_options are combined with the match options specified 
 * when the @regex structure was created, letting you have more 
 * flexibility in reusing #GivRegex structures.
 *
 * A #GivMatchInfo structure, used to get information on the match, 
 * is stored in @match_info if not %NULL. Note that if @match_info 
 * is not %NULL then it is created even if the function returns %FALSE, 
 * i.e. you must free it regardless if regular expression actually matched.
 *
 * To retrieve all the non-overlapping matches of the pattern in 
 * string you can use giv_match_info_next().
 *
 * |[
 * static void
 ...
 *   GivMatchInfo *match_info;
 *   &nbsp;
 *   regex = giv_regex_new ("[A-Z]+", 0, 0, NULL);
 *   giv_regex_match (regex, string, 0, &amp;match_info);
 *   while (giv_match_info_matches (match_info))
 *     {
 *       gchar *word = giv_match_info_fetch (match_info, 0);
 *       g_print ("Found: %s\n", word);
 *       g_free (word);
 *       giv_match_info_next (match_info, NULL);
 *     }
 ...
 * ]|
 *
 * @string is not copied and is used in #GivMatchInfo internally. If 
 * you use any #GivMatchInfo method (except giv_match_info_free()) after 
 ...
 */
gboolean
giv_regex_match (const GivRegex      *regex, 
	       const gchar       *string, 
	       GivRegexMatchFlags   match_options,
	       GivMatchInfo       **match_info)
{
  return giv_regex_match_full (regex, string, -1, 0, match_options,
			     match_info, NULL);
}

/**
 * giv_regex_match_full:
 * @regex: a #GivRegex structure from giv_regex_new()
 * @string: the string to scan for matches
 ...
 * string and setting #GIV_REGEX_MATCH_NOTBOL in the case of a pattern 
 * that begins with any kind of lookbehind assertion, such as "\b".
 *
 * A #GivMatchInfo structure, used to get information on the match, is 
 ...
 *   &nbsp;
 *   regex = giv_regex_new ("[A-Z]+", 0, 0, NULL);
 *   giv_regex_match_full (regex, string, -1, 0, 0, &amp;match_info, &amp;error);
 *   while (giv_match_info_matches (match_info))
 *     {
 *       gchar *word = giv_match_info_fetch (match_info, 0);
 *       g_print ("Found: %s\n", word);
 *       g_free (word);
 *       giv_match_info_next (match_info, &amp;error);
 *     }
 *   giv_match_info_free (match_info);
 *   giv_regex_unref (regex);
 *   if (error != NULL)
 ...
 */
gboolean
giv_regex_match_full (const GivRegex      *regex,
		    const gchar       *string,
		    gssize             string_len,
		    gint               start_position,
		    GivRegexMatchFlags   match_options,
		    ...
		    GError           **error)
{
  ...
  g_return_val_if_fail (error == NULL || *error == NULL, FALSE);
  g_return_val_if_fail ((match_options & ~GIV_REGEX_MATCH_MASK) == 0, FALSE);

  info = match_info_new (regex, string, string_len, start_position,
			 match_options, FALSE);
  match_ok = giv_match_info_next (info, error);
  if (match_info != NULL)
    *match_info = info;
  else
    giv_match_info_free (info);
 ...
 * For more documentation see giv_regex_match_all_full().
 *
 * A #GivMatchInfo structure, used to get information on the match, is 
 * stored in @match_info if not %NULL. Note that if @match_info is 
 * not %NULL then it is created even if the function returns %FALSE, 
 ...
 */
gboolean
giv_regex_match_all (const GivRegex      *regex,
		   const gchar       *string,
		   GivRegexMatchFlags   match_options,
		   GivMatchInfo       **match_info)
{
  return giv_regex_match_all_full (regex, string, -1, 0, match_options,
				 match_info, NULL);
}

/**
 * giv_regex_match_all_full:
 * @regex: a #GivRegex structure from giv_regex_new()
 * @string: the string to scan for matches
 * @string_len: the length of @string, or -1 if @string is nul-terminated
 * @start_position: starting index of the string to match
 ...
 * giv_match_info_get_match_count(). To obtain the matched strings and 
 * their position you can use, respectively, giv_match_info_fetch() and 
 * giv_match_info_fetch_pos(). Note that the strings are returned in 
 ...
 */
gboolean
giv_regex_match_all_full (const GivRegex      *regex,
			const gchar       *string,
			gssize             string_len,
			gint               start_position,
			GivRegexMatchFlags   match_options,
			...
			GError           **error)
{
  ...
  /* set info->pos to -1 so that a call to giv_match_info_next() fails. */
  info->pos = -1;
...
/**
 * giv_regex_get_string_number:
 * @regex: #GivRegex structure
 * @name: name of the subexpression
 *
 ...
 */
gint
giv_regex_get_string_number (const GivRegex *regex,
			   const gchar  *name)
{
  gint num;
 ...
 *
 * This function is equivalent to giv_regex_split() but it does 
 * not require to compile the pattern with giv_regex_new(), avoiding 
 * some lines of code when you need just to do a split without 
 * extracting substrings, capture counts, and so on.
 *
 * If this function is to be called on the same @pattern more than
 * once, it's more efficient to compile the pattern once with
 * giv_regex_new() and then use giv_regex_split().
 *
 ...
 **/
gchar **
giv_regex_split_simple (const gchar        *pattern,
		      const gchar        *string, 
		      GivRegexCompileFlags  compile_options,
		      GivRegexMatchFlags    match_options)
{
  ...
  if (!regex)
    return NULL;
  result = giv_regex_split_full (regex, string, -1, 0, match_options, 0, NULL);
  giv_regex_unref (regex);
  return result;
}
 ...
 **/
gchar **
giv_regex_split (const GivRegex     *regex, 
	       const gchar      *string, 
	       GivRegexMatchFlags  match_options)
{
  return giv_regex_split_full (regex, string, -1, 0,
                             match_options, 0, NULL);
}

/**
 * giv_regex_split_full:
 * @regex: a #GivRegex structure
 * @string: the string to split with the pattern
 * @string_len: the length of @string, or -1 if @string is nul-terminated
 * @start_position: starting index of the string to match
 ...
 **/
gchar **
giv_regex_split_full (const GivRegex      *regex, 
		    const gchar       *string, 
		    gssize             string_len,
		    gint               start_position,
		    GivRegexMatchFlags   match_options,
		    ...
		    GError           **error)
{
  ...
  match_ok = giv_regex_match_full (regex, string, string_len, start_position,
  ...
  while (tmp_error == NULL)
          ...
          if (last_separator_end != match_info->offsets[1])
            {
               ...
               * the list. */
              match_count = giv_match_info_get_match_count (match_info);
              if (match_count > 1)
                {
                  for (i = 1; i < match_count; i++)
                    list = g_list_prepend (list, giv_match_info_fetch (match_info, i));
                }
            }
        }
      else
      ...
      if (token_count >= max_tokens - 1)
	{
      ...
      if (last_match_is_empty)
        /* if the last match was empty, giv_match_info_next() has moved
      ...
      match_ok = giv_match_info_next (match_info, &tmp_error);
    }
  giv_match_info_free (match_info);
  if (tmp_error != NULL)
    {
	       ...
	       GError            **error)
{
 ...
 error:
  ...
  tmp_error = g_error_new (GIV_REGEX_ERROR, 
...
static gboolean
interpolate_replacement (const GivMatchInfo *match_info,
			 GString          *result,
			 gpointer          data)
{
  GList *list;
  ...
  for (list = data; list; list = list->next)
	...
	case REPL_TYPE_NUMERIC_REFERENCE:
	  match = giv_match_info_fetch (match_info, idata->num);
	  if (match)
	    {
	...
	case REPL_TYPE_SYMBOLIC_REFERENCE:
	  match = giv_match_info_fetch_named (match_info, idata->text);
	  if (match)
	    {
	      string_append (result, match, &change_case);
	      g_free (match);
 ...
 * </variablelist>
 *
 * If you do not need to use backreferences use giv_regex_replace_literal().
 *
 * The @replacement string must be UTF-8 encoded even if #GIV_REGEX_RAW was
 * passed to giv_regex_new(). If you want to use not UTF-8 encoded stings
 * you can use giv_regex_replace_literal().
 *
 * Setting @start_position differs from just passing over a shortened 
 * string and setting #GIV_REGEX_MATCH_NOTBOL in the case of a pattern that 
 ...
 */
gchar *
giv_regex_replace (const GivRegex      *regex, 
		 const gchar       *string, 
		 gssize             string_len,
		 gint               start_position,
		 ...
		 GError           **error)
{
  ...
  result = giv_regex_replace_eval (regex, 
...
static gboolean
literal_replacement (const GivMatchInfo *match_info,
		     GString          *result,
		     gpointer          data)
{
 ...
 * include backreferences use giv_regex_replace().
 *
 * Setting @start_position differs from just passing over a 
 * shortened string and setting #GIV_REGEX_MATCH_NOTBOL in the 
 ...
 */
gchar *
giv_regex_replace_literal (const GivRegex      *regex,
			 const gchar       *string,
			 gssize             string_len,
			 gint               start_position,
			 const gchar       *replacement,
			 ...
			 GError           **error)
{
  ...
  return giv_regex_replace_eval (regex,
			       string, string_len, start_position,
			       match_options,
			       literal_replacement,
 ...
 * that begins with any kind of lookbehind assertion, such as "\b".
 *
 * The following example uses giv_regex_replace_eval() to replace multiple
 * strings at once:
 * |[
 * static gboolean 
 * eval_cb (const GivMatchInfo *info,          
 *          GString          *res,
 *          gpointer          data)
 * {
 *   gchar *match;
 *   gchar *r;
 * 
 *    match = giv_match_info_fetch (info, 0);
 ...
 * 
 * reg = giv_regex_new ("1|2|3|4", 0, 0, NULL);
 * res = giv_regex_replace_eval (reg, text, -1, 0, 0, eval_cb, h, NULL);
 ...
 */
gchar *
giv_regex_replace_eval (const GivRegex        *regex,
		      const gchar         *string,
		      gssize               string_len,
		      gint                 start_position,
		      GivRegexMatchFlags     match_options,
		      ...
		      GError             **error)
{
  ...
  giv_regex_match_full (regex, string, string_len, start_position,
		      match_options, &match_info, &tmp_error);
  while (!done && giv_match_info_matches (match_info))
    {
      ...
      giv_match_info_next (match_info, &tmp_error);
    }
  giv_match_info_free (match_info);
  if (tmp_error != NULL)
    {
 ...
 * (see giv_regex_replace()), i.e. that all escape sequences in 
 * it are valid.
 *
 ...
 * subpattern) requires valid #GivMatchInfo object.
 *
 ...
 */
gboolean
giv_regex_check_replacement (const gchar  *replacement,
 ...
 */
gchar *
giv_regex_escape_string (const gchar *string,
		       gint         length)
{
  GString *escaped;
